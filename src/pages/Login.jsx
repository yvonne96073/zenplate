...
// Redirect to system browser, carrying ?google=1 so the app auto-triggers OAuth
function openInBrowserWithGoogle() {
  const base = window.location.origin + window.location.pathname
  const target = `${base}?google=1`
  if (isAndroid()) {
    const intent = `intent://${target.replace(/^https?:\/\//, '')}#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=${encodeURIComponent(target)};end`
    window.location.href = intent
  } else {
    window.location.href = target.replace(/^https:\/\//, 'safari-https://')
  }
}
...
export default function Login() {
  ...
  // If ?google=1 is in the URL (user arrived from in-app → Safari redirect),
  // auto-trigger Google OAuth immediately
  useEffect(() => {
  ...
  const handleGoogleLogin = () => {
    setError('')
    if (inApp) {
      // In-app browser: redirect to Safari carrying ?google=1
      // so Safari auto-triggers OAuth on arrival
      openInBrowserWithGoogle()
    } else {
      triggerGoogleOAuth()
    }
  }
